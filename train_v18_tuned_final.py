...
def train_pinn_v18_tuned(model, dataset, epochs=25000, warmup_epochs=5000):
    ...
    for epoch in range(warmup_epochs, epochs):
        ...
        # TUNING: A moderate increase in the boundary loss weight
        lambda_boundary_tuned = 250.0 
        
        total_loss = lambda_boundary_tuned * loss_boundary + 50.0 * loss_source + 100.0 * loss_sheath + 25.0 * loss_quiescence
        total_loss.backward(); optimizer.step()
