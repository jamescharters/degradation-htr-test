#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V10 (FINAL)
The definitive version. Incorporates all learned strategies: curriculum warm-up,
importance sampling, optimizer reset, target annealing, and a dedicated, lower
learning rate for the physics fine-tuning phase to ensure a stable convergence.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V10 (FINE-TUNING LR)")
print("=" * 60)
...
# PHYSICS LOSS
# ============================================
def physics_loss_v10(model, x_src, y_src, t_src, x_q, y_q, t_q, current_target_rate):
...
# TRAINING
# ============================================
def train_pinn_v10(model, dataset, epochs=15000, lr_warmup=1e-3, lr_finetune=1e-4, warmup_epochs=4000):
    
    lambda_data = 150.0; lambda_source = 50.0; lambda_quiescence = 50.0
    final_target_rate = 1.5; start_target_rate = 0.1

    print("\n" + "="*60 + f"\nTRAINING STRATEGY: V10\n" + "-"*60)
    print(f"  Phase 1: Data Warm-up ({warmup_epochs:,} epochs, LR={lr_warmup})")
    print(f"  Phase 2: Physics Fine-tuning ({epochs-warmup_epochs:,} epochs, LR={lr_finetune}) <-- NOTE REDUCED LR\n" + "="*60 + "\n")
    ...
    for epoch in range(finetune_epochs):
        ...
        loss_source, loss_quiescence = physics_loss_v10(model, x_src_t, y_src_t, t_src_t, x_q, y_q, t_q, current_target_rate)
...
# VISUALIZATION & MAIN
# ============================================
def visualize_reconstruction_v10(model, dataset, output_path='reconstruction_v10.png'):
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V10(prefix, meta_path)
    model = QuillPINN_V10().to(device)
    train_pinn_v10(model, dataset)
    visualize_reconstruction_v10(model, dataset)
