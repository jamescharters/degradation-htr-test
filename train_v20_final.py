...
This version returns to the robust and successful V18 architecture and makes the
minimalist change required to handle pressure. It scraps the flawed RateNet and
directly applies the known physical relationship (rate ∝ pressure) in the loss
function. This is the final, correct, and simplest effective model.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN V20 (DIRECT PHYSICS)")
print("=" * 60)
...
# ============================================
class QuillPINN_V20(nn.Module):
    def __init__(self, encoding_dim=10):
        super().__init__()
        self.encoding_dim = encoding_dim
        input_dim = 4 * 2 * encoding_dim # x, y, t, p
        ...
        print(f"✓ Pressure-Sensitive PINN created (Params: {sum(p.numel() for p in self.parameters()):,})")
...
class QuillDataset_V20:
    ...
    def get_final_points(self, n):
        ...
        return x, y, p, h
    def get_off_path_points(self, n):
        # Return random x,y,t and also random pressures
        return torch.rand(n, 4, device=device)
...
# TRAINING
# ============================================
def train_pinn_v20(model, dataset, epochs=25000, warmup_epochs=5000):
    ...
    for epoch in range(warmup_epochs, epochs):
        ...
        # --- THE FINAL, CORRECTED PHYSICS ---
        # The target rate is now a direct, known function of pressure.
        base_rate = 2.5 # This is a hyperparameter that maps pressure=[0,1] to a physical rate
        target_rate = base_rate * p_src
        ...
        if (epoch + 1) % 500 == 0:
            ...
            print(f"    - Boundary: {loss_boundary.item():.6f} | Source: {loss_source.item():.6f}")
...
def visualize_reconstruction_v20(model, dataset, output_path='reconstruction_v20.png'):
    ...
    with torch.no_grad():
        ...
        for i, (t_val, p_val) in enumerate(zip(vis_times, vis_pressures)):
            ...
            axes[i].set_title(f't={t_val:.2f}, p={p_val:.2f}'); axes[i].axis('off')
