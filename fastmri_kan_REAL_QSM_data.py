...
# 1. CONFIGURATION
# ==========================================
DATASET_PATH = Path("./data/OSF_QSM_Dataset/train_data")
SUBJECT_ID = "Subject_01"
SLICE_NUM = 80  # Middle slice, adjust as needed
...
def load_real_qsm_data(dataset_path, subject_id, slice_idx):
    """
    ...
    print(f"Loading real QSM data from {subject_path}...")
    
    # Find local field file (input)
    local_field_files = list(subject_path.glob("*LocalField*.nii.gz")) + \
                       list(subject_path.glob("*local_field*.nii.gz")) + \
                       list(subject_path.glob("*Field*.nii.gz")) + \
                       list(subject_path.glob("phi*.nii"))
    
    if not local_field_files:
        raise FileNotFoundError(f"No local field files found in {subject_path}")
    
    # Load first orientation
    local_field_img = nib.load(str(local_field_files[0]))
    local_field_3d = local_field_img.get_fdata()
    
    # Load COSMOS (ground truth)
    cosmos_files = list(subject_path.glob("*COSMOS*.nii.gz")) + \
                   list(subject_path.glob("*cosmos*.nii.gz"))
    
    if not cosmos_files:
        raise FileNotFoundError(f"No COSMOS files found in {subject_path}")
    
    cosmos_img = nib.load(str(cosmos_files[0]))
    cosmos_3d = cosmos_img.get_fdata()
    
    # Load or create mask
    mask_files = list(subject_path.glob("*Mask*.nii.gz")) + \
                 list(subject_path.glob("*mask*.nii.gz"))
    
    if mask_files:
        mask_img = nib.load(str(mask_files[0]))
        mask_3d = mask_img.get_fdata()
    else:
        # Create simple threshold mask
        mask_3d = (np.abs(cosmos_3d) > 0.01).astype(np.float32)
    
    # Extract 2D slice
    local_field_2d = local_field_3d[:, :, slice_idx].astype(np.float32)
    cosmos_2d = cosmos_3d[:, :, slice_idx].astype(np.float32)
    mask_2d = mask_3d[:, :, slice_idx].astype(np.float32)
    
    print(f"✓ Loaded slice {slice_idx}")
    print(f"  Local field range: [{local_field_2d.min():.3f}, {local_field_2d.max():.3f}]")
    ...
    magnitude = mask_2d  # Use mask as "magnitude" 
    phase_wrapped = local_field_2d  # Local field is like wrapped phase
    phase_true = cosmos_2d  # COSMOS is ground truth
    
    # Normalize for consistency with your original code
    # Local fields are typically in radians or Hz
    # Wrap to [-pi, pi] if needed
    phase_wrapped = np.angle(np.exp(1j * phase_wrapped))
...
mag, p_true, p_wrapped = load_real_qsm_data(DATASET_PATH, SUBJECT_ID, SLICE_NUM)
H, W = mag.shape
split_row = int(H * 0.5)
...
plt.tight_layout()
plt.savefig(f'kan_qsm_real_data_{SUBJECT_ID}_slice{SLICE_NUM}.png', dpi=150)
plt.show()

print(f"\n✓ Results saved to: kan_qsm_real_data_{SUBJECT_ID}_slice{SLICE_NUM}.png")