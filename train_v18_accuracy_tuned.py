#!/usr/bin/env python3
"""
Quill PINN V18 (ACCURACY TUNED)
This version tunes the parameters of the successful V18 script to prioritize
the accuracy of the final t=1 frame, making it as close as possible to the
ground truth.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V18 (ACCURACY TUNED)")
print("=" * 60)
...
# ============================================
# MODEL & GRADIENT UTILS (Unchanged)
# ============================================
...
# ============================================
def train_pinn_v18_tuned(model, dataset, epochs=25000, warmup_epochs=5000):
    ...
    for epoch in range(warmup_epochs):
        ...
        n_data = 2048
        ic_pts = dataset.get_initial_points(n_data); x_ic, y_ic = ic_pts[:, 0], ic_pts[:, 1]
    ...
    for epoch in range(warmup_epochs, epochs):
        ...
        # TUNING 1: Increase the number of boundary points for a clearer signal
        n_data = 2048
        ic_pts = dataset.get_initial_points(n_data); x_ic, y_ic = ic_pts[:, 0], ic_pts[:, 1]
        loss_ic = (model(x_ic, y_ic, torch.zeros_like(x_ic)) ** 2).mean()
        x_fc, y_fc, h_fc = dataset.get_final_points(n_data)
        ...
        off_path_pts = dataset.get_off_path_points(n_phys); x_q, y_q, t_q = off_path_pts[:, 0], off_path_pts[:, 1], off_path_pts[:, 2]
        ...
        # TUNING 2: Increase the weight of the boundary loss to prioritize it
        lambda_boundary_tuned = 300.0
        
        total_loss = (lambda_boundary_tuned * loss_boundary + 
                      50.0 * loss_source + 
                      100.0 * loss_sheath + 
                      25.0 * loss_quiescence)
        total_loss.backward(); optimizer.step()
...
def visualize_reconstruction_v18_tuned(model, dataset, output_path='reconstruction_v18_tuned.png'):
    ...
    with torch.no_grad():
        for i, t_val in enumerate(time_points):
            x_grid, y_grid = torch.linspace(0,1,dataset.width,device=device); y_grid = torch.linspace(0,1,dataset.height,device=device)
...
if __name__ == "__main__":
    ...
    model = QuillPINN_V18(encoding_dim=10) # Using encoding_dim=10 as it's a good default
    train_pinn_v18_tuned(model, dataset)
