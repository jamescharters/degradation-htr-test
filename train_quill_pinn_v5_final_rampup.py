#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V5 (FINAL RAMP-UP)
This version implements a "curriculum learning" strategy with a warm-up phase
and a gradual ramp-up of physics losses to ensure stable convergence.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V5 (GRADUAL RAMP-UP)")
print("=" * 60)
...
def physics_loss_v5_sourceterm(model, dataset, x, y, t):
    ...
    target_rate = 1.0 # CHANGED: Reduced from 2.0 to be less aggressive
...
# ============================================
# TRAINING (with Curriculum and Ramp-up)
# ============================================
def train_pinn_v5(model, dataset, epochs=15000, lr=1e-3, warmup_epochs=3000, ramp_epochs=2000):
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    
    lambda_data_final = 150.0
    lambda_source_final = 20.0
    lambda_quiescence_final = 20.0

    print("\n" + "="*60 + f"\nTRAINING STRATEGY: CURRICULUM & RAMP-UP\n" + "-"*60)
    print(f"  Phase 1: Data Warm-up     ({warmup_epochs:,} epochs)")
    print(f"  Phase 2: Physics Ramp-up    ({ramp_epochs:,} epochs)")
    print(f"  Phase 3: Full Physics       ({epochs-warmup_epochs-ramp_epochs:,} epochs)\n" + "="*60 + "\n")
    ...
    for epoch in range(epochs):
        ...
        # --- NEW: RAMP-UP LOGIC ---
        current_lambda_source = 0.0
        current_lambda_quiescence = 0.0
        
        if epoch >= warmup_epochs:
            # Calculate the progress of the ramp-up phase
            ramp_progress = min(1.0, (epoch - warmup_epochs) / ramp_epochs)
            current_lambda_source = lambda_source_final * ramp_progress
            current_lambda_quiescence = lambda_quiescence_final * ramp_progress

        if current_lambda_source > 0:
            ...
            total_loss = (lambda_data_final * loss_boundary +
                          current_lambda_source * loss_src +
                          current_lambda_quiescence * loss_q)
        else: # WARM-UP PHASE
            total_loss = lambda_data_final * loss_boundary
        ...
        if (epoch + 1) % 500 == 0:
            phase = "WARM-UP"
            if epoch >= warmup_epochs + ramp_epochs: phase = "FULL PHYSICS"
            elif epoch >= warmup_epochs: phase = "RAMP-UP"

            print(f"--- Epoch {epoch+1:5d}/{epochs} [PHASE: {phase}] ---")
            print(f"  TOTAL LOSS: {total_loss.item():.6f}")
            print(f"    - Boundary Loss (Weighted): {(lambda_data_final * loss_boundary).item():.6f}")
            if phase != "WARM-UP":
                print(f"    - Source Loss (Weighted):   {(current_lambda_source * loss_src).item():.6f} (λ={current_lambda_source:.2f})")
                print(f"    - Quiescence Loss (Weighted): {(current_lambda_quiescence * loss_q).item():.6f} (λ={current_lambda_quiescence:.2f})")
