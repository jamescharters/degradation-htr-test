...
def apply_growth_mask(model, current_width):
    """
    Forces the model to act as if it only has 'current_width' channels.
    We zero out weights for indices >= current_width.
    """
    # 1. Zero out weights for "future" channels (Simulate them not existing)
    with torch.no_grad():
        # Block 3 Conv (Index 6)
        model.block3[6].weight.data[current_width:, :, :, :] = 0.0
        if model.block3[6].bias is not None:
            model.block3[6].bias.data[current_width:] = 0.0
            
        # BCA Weights (Input connections)
        model.bca.fc_shared[0].weight.data[:, current_width:, :, :] = 0.0


def train_with_growth(model, train_loader, val_loader, device):
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    ...
    UNCERTAINTY_THRESHOLD = 0.85  # If variance is below this, the current brain is "stable"
    ACC_THRESHOLD = 96.0         # If acc is below this, we still need to learn more
    
    history_width = []
    history_acc = []
    history_unc = []
    
    print(f"--- Starting Dynamic Growth (Initial Width: {current_width}) ---")
    
    # Train in "Cycles"
    for cycle in range(1, 10): # Max 10 growth cycles
        print(f"\n[Cycle {cycle}] Current Width: {current_width} channels")
        
        # Train for a few epochs to let the current width converge
        for epoch in range(3):
            ...
            for inputs, labels, _ in train_loader:
                ...
                # --- CONSTRAINT: Force "Future" channels to zero ---
                apply_growth_mask(model, current_width)
                
                logits, mu, log_var, _ = model(inputs)
                loss = elbo_loss(logits, labels, mu, log_var)
                loss.backward()
                
                # --- CONSTRAINT: Zero out Gradients for "Future" channels ---
                # This prevents them from updating/learning yet
                with torch.no_grad():
                    model.block3[6].weight.grad.data[current_width:, :, :, :] = 0.0
                    model.block3[6].bias.grad.data[current_width:] = 0.0
        ...
        with torch.no_grad():
            for inputs, labels, _ in val_loader:
                inputs, labels = inputs.to(device), labels.to(device)
                apply_growth_mask(model, current_width) # Ensure mask applies during val
        ...
        # 2. Check Uncertainty of the ACTIVE channels
        # We assume indices 0 to current_width are the active ones
        # We just grab the mean uncertainty of ALL, because the inactive ones are 0/frozen
        # but technically we should slice.
        _, avg_vars = profile_channel_uncertainty(model, val_loader, device)
        # Average variance of only the ACTIVE channels
        active_uncertainty = avg_vars[:current_width].mean().item()
        
        print(f"   -> Accuracy: {val_acc:.2f}% | Active Uncertainty: {active_uncertainty:.4f}")
        
        history_width.append(current_width)
        history_acc.append(val_acc)
        history_unc.append(active_uncertainty)
        
        # --- DECISION LOGIC (The "Brain Growth" AI) ---
        if val_acc >= ACC_THRESHOLD:
            print("   -> Target Accuracy Reached! Stopping growth.")
            break
        ...
        # The Core Hypothesis Check:
        # If Uncertainty is LOW, it means the current channels are confident/stable.
        # If we are stable but Accuracy is still low, it means we lack CAPACITY.
        # Therefore -> GROW.
        if active_uncertainty < UNCERTAINTY_THRESHOLD:
            print(f"   -> Model is Stable (Unc {active_uncertainty:.2f} < {UNCERTAINTY_THRESHOLD}) but Underperforming. GROWING BRAIN.")
            current_width += step_size
        else:
            print(f"   -> Model is still High Variance ({active_uncertainty:.2f}). Needs more training before growing.")
            
    return history_width, history_acc, history_unc
...
if __name__ == "__main__":
    ...
    widths, accs, uncs = train_with_growth(model, train_loader, val_loader, device)
    
    # Plot
    fig, ax1 = plt.subplots()
    ax1.set_xlabel('Cycle')
    ax1.set_ylabel('Accuracy', color='blue')
    ax1.plot(accs, color='blue', marker='o', label='Accuracy')
    
    ax2 = ax1.twinx()
    ax2.set_ylabel('Width (Channels)', color='green')
    ax2.plot(widths, color='green', marker='x', linestyle='--', label='Width')
    
    plt.title("Dynamic Brain Growth: Accuracy vs Capacity")
    plt.show()