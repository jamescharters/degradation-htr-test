#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V13 (FINAL)
The definitive version. Implements a three-phase curriculum to solve the training
deadlock: 1) Boundary Warm-up, 2) Source-Only Activation to escape the local
minimum, and 3) Full Physics Polish to refine the result.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V13 (THREE-PHASE CURRICULUM)")
print("=" * 60)
...
# TRAINING
# ============================================
def train_pinn_v13(model, dataset, epochs=25000, phases=[5000, 10000, 10000]):
    ...
    for epoch in range(phases[0]):
        ...
        if (epoch + 1) % 500 == 0: print(f"  Epoch {epoch+1:5d}/{phases[0]} | Warm-up Loss: {total_loss.item():.6f}")

    # Phase 2: Source Activation
    print("\n" + "="*60 + f"\nPHASE 2: SOURCE ACTIVATION ({phases[1]:,} epochs)\n" + "-"*60)
    optimizer = torch.optim.Adam(model.parameters(), lr=5e-4) # Fresh optimizer
    for epoch in range(phases[1]):
        ...
        total_loss = 50.0 * loss_source
        total_loss.backward(); optimizer.step()
        if (epoch + 1) % 500 == 0: print(f"  Epoch {epoch+1+phases[0]:5d}/{epochs} | Source Activation Loss: {total_loss.item():.6f} (Raw: {loss_source.item():.6f})")

    # Phase 3: Full Physics Polish
    print("\n" + "="*60 + f"\nPHASE 3: FULL PHYSICS POLISH ({phases[2]:,} epochs)\n" + "-"*60)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4) # Fresh optimizer, low LR
    for epoch in range(phases[2]):
        model.train(); optimizer.zero_grad()
        # Boundary loss (low weight, just to prevent drift)
        n_data = 512
        ic_pts = dataset.get_initial_points(n_data); x_ic, y_ic = torch.tensor(ic_pts[:,0],dtype=torch.float32,device=device), torch.tensor(ic_pts[:,1],dtype=torch.float32,device=device)
        loss_ic = (model(x_ic, y_ic, torch.zeros_like(x_ic)) ** 2).mean()
        x_fc, y_fc, h_fc = dataset.get_final_points(n_data); x_fc_t, y_fc_t, h_fc_t = torch.tensor(x_fc,dtype=torch.float32,device=device), torch.tensor(y_fc,dtype=torch.float32,device=device), torch.tensor(h_fc,dtype=torch.float32,device=device)
        ...
        x_src, y_src, t_src = dataset.get_on_path_points(n_source_pts); x_src_t, y_src_t, t_src_t = torch.tensor(x_src,dtype=torch.float32,device=device,requires_grad=True), torch.tensor(y_src,dtype=torch.float32,device=device,requires_grad=True), torch.tensor(t_src,dtype=torch.float32,device=device,requires_grad=True)
        h_src = model(x_src_t, y_src_t, t_src_t); h_t_src = compute_gradient(h_src, t_src_t)
        ...
        total_loss = 25.0 * loss_boundary + 50.0 * loss_source + 50.0 * loss_quiescence
        total_loss.backward(); optimizer.step()
...
# VISUALIZATION & MAIN
# ============================================
def visualize_reconstruction_v13(model, dataset, output_path='reconstruction_v13.png'):
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V13(prefix, meta_path)
    model = QuillPINN_V13().to(device)
    train_pinn_v13(model, dataset)
    visualize_reconstruction_v13(model, dataset)
