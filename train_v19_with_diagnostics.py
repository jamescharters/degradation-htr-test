#!/usr/bin/env python3
"""
Quill PINN V19 (WITH DIAGNOSTICS)
This version extends the successful V19 model to produce a comprehensive
diagnostic visualization that explains the internal workings of the model,
including the learned RateNet response curve and the dynamic ∂h/∂t spark.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN V19 (WITH DIAGNOSTICS)")
print("=" * 60)
...
# ============================================
# NOTE: Renamed QuillPINN to avoid conflicts if you run both scripts
class QuillPINN_V19_Diag(nn.Module):
...
def train_pinn_v19(model, dataset, epochs=25000, warmup_epochs=5000):
    ...
    for epoch in range(warmup_epochs):
        ...
        if (epoch + 1) % 1000 == 0: print(f"  Epoch {epoch+1:5d}/{epochs} | Warm-up Loss: {total_loss.item():.6f}")
    ...
    for epoch in range(warmup_epochs, epochs):
        ...
        # This is a simplification; a true integral loss is more complex but this works well.
        loss_rate_net = ((h_src.detach() - rate_pred_for_src)**2).mean()

        total_loss = (150.0*loss_boundary + 50.0*loss_source + 100.0*loss_sheath + 25.0*loss_quiescence + 10.0*loss_rate_net)
        total_loss.backward(); optimizer.step()
        
        if (epoch + 1) % 1000 == 0:
            print(f"  Epoch {epoch+1:5d}/{epochs} | Total Loss: {total_loss.item():.4f} | Source: {loss_source.item():.6f} | RateNet: {loss_rate_net.item():.6f}")
...
def visualize_diagnostics_v19(model, dataset, output_path='reconstruction_v19_diagnostics.png'):
    ...
    with torch.no_grad():
        ...
        for i, (t_val, p_val) in enumerate(zip(vis_times, vis_pressures)):
            ...
            ax.imshow(1.0 - h_pred, cmap='gray', vmin=0, vmax=1)
            ax.set_title(f'Reconstruction h\nt={t_val:.2f}, p={p_val:.2f}'); ax.axis('off')

    # --- Part 2: Visualization of ∂h/∂t (The "Spark") ---
    for i, (t_val, p_val) in enumerate(zip(vis_times, vis_pressures)):
        ax = fig.add_subplot(gs[1, i])
        t_flat = torch.full_like(x_flat, t_val, requires_grad=True)
        p_flat = torch.full_like(x_flat, p_val)
        h_pred = model(x_flat, y_flat, t_flat, p_flat)
        h_t_pred = compute_gradient(h_pred, t_flat)
        h_t_img = h_t_pred.detach().cpu().numpy().reshape(dataset.height, dataset.width)
        
        im = ax.imshow(h_t_img, cmap='RdBu_r', vmin=-2.0, vmax=2.0, origin='lower')
        ax.set_title(f'∂h/∂t (The Spark)\nt={t_val:.2f}'); ax.axis('off')

    # --- Part 3: The Learned Physics (RateNet Curve) ---
    ...
    with torch.no_grad():
        ...
        ax_rate.set_xlabel("Input Pressure (Normalized)", fontsize=12)
        ax_rate.set_ylabel("Learned Target Rate for ∂h/∂t", fontsize=12)
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V19_Diag(prefix, meta_path)
    model = QuillPINN_V19_Diag().to(device)
