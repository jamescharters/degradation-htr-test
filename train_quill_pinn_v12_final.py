#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V12 (FINAL)
The definitive version. Solves the training collapse by fully decoupling the
training tasks. Phase 1 learns the boundary conditions. Phase 2 turns OFF the
boundary loss and learns ONLY the physics of the transition.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V12 (DECOUPLED TASKS)")
print("=" * 60)
...
# TRAINING
# ============================================
def train_pinn_v12(model, dataset, epochs=20000, lr_warmup=1e-3, lr_finetune=5e-5, warmup_epochs=5000):
    
    lambda_data = 200.0; lambda_source = 1.0; lambda_quiescence = 1.0
    
    print("\n" + "="*60 + f"\nTRAINING STRATEGY: V12 (DECOUPLED)\n" + "-"*60)
    print(f"  Phase 1: Boundary Warm-up ({warmup_epochs:,} epochs, LR={lr_warmup})")
    print(f"  Phase 2: Physics-Only Fine-tuning ({epochs-warmup_epochs:,} epochs, LR={lr_finetune})\n" + "="*60 + "\n")
    
    # --- PHASE 1: WARM-UP ---
    print("--- Starting Phase 1: BOUNDARY WARM-UP ---")
    ...
    print("\n--- Starting Phase 2: PHYSICS-ONLY FINE-TUNING ---")
    ...
    for epoch in range(finetune_epochs):
        ...
        n_source_pts, n_quiescence_pts = 2048, 2048
        ...
        # *** THE CRITICAL CHANGE ***
        # The total loss is NOW ONLY the physics. The boundary conditions
        # have already been learned and are now "baked in" to the weights.
        total_loss = (lambda_source * loss_source +
        ...
        if (epoch + 1) % 500 == 0:
            ...
            print(f"    - Raw Source Loss: {loss_source.item():.6f} | Raw Quiescence Loss: {loss_quiescence.item():.6f}")
...
# VISUALIZATION & MAIN
# ============================================
def visualize_reconstruction_v12(model, dataset, output_path='reconstruction_v12.png'):
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V12(prefix, meta_path)
    model = QuillPINN_V12().to(device)
    train_pinn_v12(model, dataset)
    visualize_reconstruction_v12(model, dataset)
