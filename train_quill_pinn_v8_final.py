#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V8 (FINAL)
This version solves the "stuck local minimum" problem by re-initializing the
optimizer for the physics fine-tuning phase. This discards the stale momentum
from the warm-up and allows the model to properly learn the new physics task.
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V8 (OPTIMIZER RESET)")
print("=" * 60)
...
# TRAINING
# ============================================
def train_pinn_v8(model, dataset, epochs=12000, lr_warmup=1e-3, lr_finetune=5e-4, warmup_epochs=4000):
    
    lambda_data = 150.0
    lambda_source = 25.0
    lambda_quiescence = 25.0

    print("\n" + "="*60 + f"\nTRAINING STRATEGY: V8\n" + "-"*60)
    print(f"  Phase 1: Data Warm-up ({warmup_epochs:,} epochs, LR={lr_warmup})")
    print(f"  Phase 2: Physics Fine-tuning ({epochs-warmup_epochs:,} epochs, LR={lr_finetune})\n" + "="*60 + "\n")
    ...
    for epoch in range(warmup_epochs):
        ...
        if (epoch + 1) % 500 == 0:
            print(f"  Epoch {epoch+1:5d}/{warmup_epochs} | Warm-up Loss: {total_loss.item():.6f}")

    # --- PHASE 2: FINE-TUNING ---
    print("\n--- Starting Phase 2: PHYSICS FINE-TUNING ---")
    # *** CRITICAL STEP: Re-initialize the optimizer ***
    optimizer = torch.optim.Adam(model.parameters(), lr=lr_finetune)

    for epoch in range(warmup_epochs, epochs):
        model.train(); optimizer.zero_grad()
        
        # Boundary losses are still calculated
        n_data = 800
        ic_pts = dataset.get_initial_points(n_data)
        x_ic, y_ic = torch.tensor(ic_pts[:,0],dtype=torch.float32,device=device), torch.tensor(ic_pts[:,1],dtype=torch.float32,device=device)
        loss_ic = (model(x_ic, y_ic, torch.zeros_like(x_ic)) ** 2).mean()
        ...
        if (epoch + 1) % 500 == 0:
            print(f"  Epoch {epoch+1:5d}/{epochs} | Fine-tune Loss: {total_loss.item():.6f}")
...
# VISUALIZATION & MAIN
# ============================================
def visualize_reconstruction_v8(model, dataset, output_path='reconstruction_v8.png'):
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V8(prefix, meta_path)
    model = QuillPINN_V8().to(device)
    train_pinn_v8(model, dataset)
    visualize_reconstruction_v8(model, dataset)
