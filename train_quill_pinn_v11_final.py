#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V11 (FINAL)
The definitive version. Solves the "learning lag" by giving the model more time
(epochs) to adapt during the physics fine-tuning phase, ensuring a stable
convergence where all losses can be minimized together.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V11 (EXTENDED FINE-TUNING)")
print("=" * 60)
...
# PHYSICS LOSS
# ============================================
def physics_loss_v11(model, x_src, y_src, t_src, x_q, y_q, t_q, current_target_rate):
...
# TRAINING
# ============================================
def train_pinn_v11(model, dataset, epochs=25000, lr_warmup=1e-3, lr_finetune=2e-4, warmup_epochs=5000):
    
    lambda_data = 150.0; lambda_source = 75.0; lambda_quiescence = 75.0
    final_target_rate = 1.5; start_target_rate = 0.1

    print("\n" + "="*60 + f"\nTRAINING STRATEGY: V11\n" + "-"*60)
    ...
    for epoch in range(finetune_epochs):
        ...
        loss_source, loss_quiescence = physics_loss_v11(model, x_src_t, y_src_t, t_src_t, x_q, y_q, t_q, current_target_rate)
...
# VISUALIZATION & MAIN
# ============================================
def visualize_reconstruction_v11(model, dataset, output_path='reconstruction_v11.png'):
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V11(prefix, meta_path)
    model = QuillPINN_V11().to(device)
    train_pinn_v11(model, dataset)
    visualize_reconstruction_v11(model, dataset)
