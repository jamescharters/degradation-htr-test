#!/usr/bin/env python3
"""
Physics-Informed Neural Network for Quill Writing Reconstruction V6
This version uses IMPORTANCE SAMPLING for the physics loss. It samples one set
of points directly on the quill path to ensure a strong source signal, and another
set off-path to enforce quiescence, solving the sparse signal problem.
"""
...
print("=" * 60)
print("QUILL PHYSICS PINN TRAINING V6 (IMPORTANCE SAMPLING)")
print("=" * 60)
...
# ============================================
# DATASET (with new on-path sampling method)
# ============================================
class QuillDataset_V6:
    ...
    # --- NEW METHOD FOR IMPORTANCE SAMPLING ---
    def get_on_path_points(self, n):
        # Sample times only during active strokes
        stroke_duration = 0
        for s in self.metadata['strokes']: stroke_duration += s['end_time'] - s['start_time']
        ...
        # Get quill position at these times
        quill_pos = self.get_quill_position_at_time(torch.from_numpy(t_on_path)).numpy()
        
        # Add a tiny bit of noise to sample *around* the path, not just on it
        noise = np.random.randn(n, 2) * (2.0 / self.width) # 2-pixel radius noise
...
# ============================================
def train_pinn_v6(model, dataset, epochs=10000, lr=1e-3):
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    
    lambda_data = 150.0
    lambda_source = 25.0 # We can be confident in this loss now
    lambda_quiescence = 25.0

    print("\n" + "="*60 + f"\nTRAINING STRATEGY: IMPORTANCE SAMPLING\n" + "="*60 + "\n")
    ...
    for epoch in range(epochs):
        ...
        # --- Physics Loss via Importance Sampling ---
        # 1. ON-PATH points for the SOURCE term
        n_source_pts = 1500
        x_src, y_src, t_src = dataset.get_on_path_points(n_source_pts)
        x_src_t, y_src_t, t_src_t = torch.tensor(x_src,dtype=torch.float32,device=device,requires_grad=True), torch.tensor(y_src,dtype=torch.float32,device=device,requires_grad=True), torch.tensor(t_src,dtype=torch.float32,device=device,requires_grad=True)
        h_src = model(x_src_t, y_src_t, t_src_t)
        h_t_src = compute_gradient(h_src, t_src_t)
        target_rate = 1.5
        loss_source = (torch.relu(target_rate - h_t_src)**2).mean()

        # 2. OFF-PATH points for the QUIESCENCE term
        n_quiescence_pts = 1500
        off_path_pts = dataset.get_off_path_points(n_quiescence_pts)
        x_q, y_q, t_q = torch.tensor(off_path_pts[:,0],dtype=torch.float32,device=device,requires_grad=True), torch.tensor(off_path_pts[:,1],dtype=torch.float32,device=device,requires_grad=True), torch.tensor(off_path_pts[:,2],dtype=torch.float32,device=device,requires_grad=True)
        ...
        if (epoch + 1) % 500 == 0:
            ...
            print(f"    - Boundary Loss:   {loss_boundary.item():.6f} (Weighted: {(lambda_data*loss_boundary).item():.4f})")
            print(f"    - Source Loss:     {loss_source.item():.6f} (Weighted: {(lambda_source*loss_source).item():.4f})")
            print(f"    - Quiescence Loss: {loss_quiescence.item():.6f} (Weighted: {(lambda_quiescence*loss_quiescence).item():.4f})")
            print("")
...
# VISUALIZATION & MAIN
# ============================================
def visualize_reconstruction_v6(model, dataset, output_path='reconstruction_v6.png'):
...
if __name__ == "__main__":
    ...
    dataset = QuillDataset_V6(prefix, meta_path)
    model = QuillPINN_V6().to(device)
    train_pinn_v6(model, dataset)
    visualize_reconstruction_v6(model, dataset)
